...
sequelize.authenticate()
  .then(() => {
    ...
    if (process.env.VERCEL) {
      console.log('Database sync skipped on Vercel serverless');
      return null;
    }
    return sequelize.sync({ alter: true });
  })
  .then(() => {
    if (!process.env.VERCEL) console.log('Database tables synced');
    initCronJobs();
  })
  .catch(err => {
    console.error('Database startup error:', err.name, err.message);
    if (err.parent) console.error('Database parent error:', err.parent.code, err.parent.message);
  });
